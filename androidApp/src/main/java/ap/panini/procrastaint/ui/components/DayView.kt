...
fun DayView(
    ...
    LazyColumn(modifier.fillMaxHeight(), verticalArrangement = Arrangement.SpaceEvenly) {

        item {
            DividerText(Date.getTodayStart().formatMilliseconds(setOf(Time.HOUR,Time.MINUTE), smart = false))
        }
        ...
        (1 until 24).forEach { hour ->
            ...
            item {
                DividerText((Date.getTodayStart() + hour.hours.inWholeMilliseconds).formatMilliseconds(setOf(Time.HOUR,Time.MINUTE), smart = false))
            }
